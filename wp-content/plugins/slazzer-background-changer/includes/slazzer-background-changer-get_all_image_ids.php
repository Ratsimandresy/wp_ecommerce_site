...
add_action('wp_ajax_nopriv_get_all_image_ids', 'get_all_image_ids');
...
function get_all_image_ids(){
    ...
    $delete_query = 'delete from `'.$wpdb->prefix.'wc_slazzer_process_status` where status="0"';
            ...
            if(isset($product_object) && $product_object != '') {
                ...
                if (!empty($image_id)) {
                    ...
                    if ($is_include_processed_image == 'TRUE') {
                        ...
                        if (!is_image_processed($product_id, $image_id)) {
                    ...
                    foreach($variations as $variation_id){
                        ...
                        if($thumbnail_id > 0){
                            ...
                            if ($is_include_processed_image == 'TRUE') {
                                ...
                                if (!is_image_processed($variation_id, $image_id)) {
                                    $wpdb->insert($table_name, $product_status_insert_args);  
                                }
                            }
                        }
                        ...
                        if (!empty($attachment_id)) {
                            ...
                            if ($is_include_processed_image == 'TRUE') {
                              ...
                              $wpdb->insert($table_name, $attachment_args); 
                               ...
                               if (!is_image_processed($product_id, $attachment_id)) {
    ...
    if(count($res) > 0){
        ...
        $sql = "UPDATE `".$wpdb->prefix."wc_slazzer_process_history` SET `can_restore_or_delete` = 0";
...
function is_image_processed($product_id, $image_id)

{

     global $wpdb;

    $sql = "SELECT * FROM `" . $wpdb->prefix . "wc_slazzer_process_history` WHERE product_id= ".$product_id." AND attach_id = ".$image_id;
