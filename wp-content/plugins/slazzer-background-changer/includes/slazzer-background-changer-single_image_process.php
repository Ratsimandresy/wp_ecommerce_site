...
add_action('wp_ajax_start_processing_single_image', 'start_processing_single_image');

add_action('wp_ajax_nopriv_start_processing_single_image', 'start_processing_single_image');



function start_processing_single_image(){
    ...
    $sql = "SELECT * FROM `".$wpdb->prefix."wc_slazzer_process_status` WHERE `status` = 0 ORDER BY id ASC LIMIT 1";
    ...
    if(count($res) > 0){
        ...
        $arr_media_image = get_base64_image($image_id);

        $old_image_file_name = $arr_media_image['old_image_file_name'];



        $arr_request_body = get_request_body($arr_media_image,$old_image_file_name);
        ...
        $arr_response_body = remove_image_background($arr_request_body,$slazzer_api_key);
        ...
        if ($arr_response_body['status']) {
            ...
            $sql = "UPDATE `".$wpdb->prefix."wc_slazzer_process_status` SET `status` = 1 WHERE id=".$id;
            ...
            $attach_id = download_image_from_url($image_url, $product_id,$old_image_file_name);
            ...
            get_remaining_credits($slazzer_api_key,API_ACCOUNT);              

            $remaining_credits = get_option('remaining_credits');           

            $sql = "SELECT * FROM `".$wpdb->prefix."wc_slazzer_process_status` WHERE `status` = 0";

            $res = $wpdb->get_results($sql );

            $img_count = count($res);

            echo json_encode(array('status' => true, 'message' => 'success', 'count' => $img_count,'remaining_credits' => $remaining_credits));
            ...
            $remaining_credits = get_option('remaining_credits');
            echo json_encode(array('status' => false, 'message' => 'Unable to process this image.','count' => $img_count,'remaining_credits' => $remaining_credits));
        ...
        $remaining_credits = get_option('remaining_credits');  
        echo json_encode(array('status' => false, 'message' => 'Done', 'count' => 0,'remaining_credits' => $remaining_credits ));
